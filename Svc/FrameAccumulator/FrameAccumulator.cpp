...
void FrameAccumulator ::processBuffer(Fw::Buffer& buffer) {
    ...
    for (FwSizeType i = 0; i < bufferSize; ++i) {
        ...
        if (remaining == 0 || this->m_inRing.getSerializeSizeLeft() == 0) {
            break;
        }
    ...
    FW_ASSERT(remaining == 0 || this->m_inRing.getSerializeSizeLeft() == 0, static_cast<FwAssertArgType>(remaining));
}
...
void FrameAccumulator ::processRing() {
    ...
    const FwSizeType ringCapacity = this->m_inRing.getCapacity();
    ...
    for (FwSizeType i = 0; i < ringCapacity; i++) {
        ...
        remaining = this->m_inRing.getSize();
        ...
        FW_ASSERT(m_inRing.getSize() == remaining,
                  static_cast<FwAssertArgType>(m_inRing.getSize()), static_cast<FwAssertArgType>(remaining));
        ...
        if (status == FrameDetector::FRAME_DETECTED) {
            ...
            if (buffer.isValid()) {
                ...
                FW_ASSERT(serialize_status == Fw::SerializeStatus::FW_SERIALIZE_OK);
                FW_ASSERT(m_inRing.getSize() == remaining - size_out,
                          static_cast<FwAssertArgType>(m_inRing.getSize()),
        ...
        // More data needed
            ...
            FW_ASSERT(size_out < m_inRing.getCapacity(), static_cast<FwAssertArgType>(size_out));
        ...
        // No frame was detected or an unknown status was received
            ...
            FW_ASSERT(m_inRing.getSize() == remaining - 1,
                      static_cast<FwAssertArgType>(m_inRing.getSize()),
                      static_cast<FwAssertArgType>(remaining));
        }
    }
